...
1. **Fixed schedule**: Pre-defined hint percentage changes at specific steps
...
- Results:
  `{fileroot}/logs/{user}/{experiment_name}/{trial_name}/rollout/0/{task_id}.jsonl`
...
All use system prompt: "Please reason step by step, and put your final answer within
\\boxed{}."
...
- **HF format** (for inference/publishing):
  `{fileroot}/checkpoints/{user}/{experiment}/{trial}/default/epoch{E}epochstep{S}globalstep{G}/`
...
- **Push**: NOT implemented in codebase. Checkpoints are saved in HF format on disk but
  not uploaded. To push:
  ```python
...
- **Modules**: `StdEnv/2023`, `python/3.10.13`, `python/3.12.4`,
  `cuda/12.2|12.6|12.9|13.2`
- **Quotas (fengdic)**:
  - Home: 50GB / 500K files
  - Scratch: 20TB / 1M files
  - Project (def-ashique): 1TB / 500K files
- **GRES syntax**: `--gres=gpu:h100:N` (NOT `gpu:l40s:N`)
- **H100 vs L40S**: H100 has 80GB VRAM (vs 48GB L40S), so `max_concurrent_rollouts` can
  be higher. Estimate ~40-50 per H100 GPU for 1.5B model.
...
1. **OpenCode extra filtering**: Evaluating `nvidia/OpenReasoning-Nemotron-1.5B` on code
   dataset (TODO in setup.md)
1. **HuggingFace push**: No automated upload — need utility script
1. **Heldout eval slurm scripts**: Need generation for AIME24/25, OlympiadBench, HMMT,
   BRUMO
1. **AIME25, HMMT, BRUMO**: No ground truth answers — evaluation generates responses but
   can't auto-score
