...
class OPSDWorkflow(RolloutWorkflow):
    """Single-turn self-distillation learning workflow supporting optional thinking tokens."""
    ...
    async def arun_episode(
        ...
        hint_input_ids = self.get_input_ids_fn(
        ...
        hint_seq = hint_input_ids + resp.output_tokens
        logprobs = [0.0] * resp.input_len + resp.output_logprobs
        loss_mask = [0] * resp.input_len + [1] * resp.output_len
        hint_loss_mask = [0] * len(hint_input_ids) + [1] * resp.output_len
        versions = [-1] * resp.input_len + resp.output_versions

        res = {
            "input_ids": torch.tensor(seq, dtype=torch.int32),
            "hint_input_ids": torch.tensor(hint_seq, dtype=torch.int32),
            "loss_mask": torch.tensor(loss_mask, dtype=torch.int32),
            "hint_loss_mask": torch.tensor(hint_loss_mask, dtype=torch.int32),
